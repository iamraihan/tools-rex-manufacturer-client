...
const MyOrders = () => {
    ...
    return (
        <div>
            {
                orders.map((order, index) => <MyOrderList key={order._id} order={order} index={index} setDeleteOrder={setDeleteOrder} ></MyOrderList>)
            }
            {
                deleteOrder && <DeleteOrderConfirmation deleteOrder={deleteOrder} setDeleteOrder={setDeleteOrder}></DeleteOrderConfirmation>
            }
        </div>
    );
};
